		...
		while self._running do
			...
			if isColossalRaid then
				...
				if isColossalPhase1 then
					UpdateStatus("Phase 1: Killing Titans + Cannon + Defending Eren")

					-- Get all walls cannons for multi-cannon fire
					local function getAllCannons()
						local result = {}
						local walls = workspace:FindFirstChild("Climbable") and workspace.Climbable:FindFirstChild("Walls")
						if not walls then return result end
					...
					-- 1. CANNON FIRE — direct remote, no mount needed, multi-hit
					if colossalNape and not colossalDead then
						task.spawn(function()
							if getgenv()._cannonFiring then return end
							getgenv()._cannonFiring = true
							local allCannons = getAllCannons()
							-- Fire 30 times rapidly from all cannons
							for _ = 1, 30 do
								for _, cannon in ipairs(allCannons) do
									pcall(function()
										-- Claim cannon
										getRemote:InvokeServer("Cannon", "Claim", cannon)
										-- Fire directly at Colossal
					...
					-- 2. DEFEND EREN + kill titans (multi-hit)
					...
					-- Hover near Eren
					if erenPos then
						local targetHeightPos = erenPos + Vector3.new(0, getgenv().AutoFarmConfig.HeightOffset, 30)
					...
					-- Multi-hit ALL titans + Colossal nape simultaneously
					if (now - lastAttack) >= getgenv().AutoFarmConfig.AttackCooldown then
						lastAttack = now

						-- Hit all valid napes (titans near Eren)
						...
						-- Also multi-hit Colossal Nape directly (15 hits)
						if colossalNape and not colossalDead then
							if slotData.Weapon == "Blades" then
								for _ = 1, 15 do
									postRemote:FireServer("Attacks", "Slash", true)
