		...
		while self._running do
			...
			if isColossalRaid then
				...
				if isColossalPhase1 then
					-- Cycle timer init
					if not getgenv()._phase1Timer then
						getgenv()._phase1Timer = now
						getgenv()._phase1Mode = "cannon" -- "cannon" or "defend"
					end

					local elapsed = now - getgenv()._phase1Timer

					-- Switch modes based on time
					if getgenv()._phase1Mode == "cannon" and elapsed >= 2 then
						getgenv()._phase1Mode = "defend"
						getgenv()._phase1Timer = now
						getgenv()._cannonFiring = false
					elseif getgenv()._phase1Mode == "defend" and elapsed >= 5 then
						getgenv()._phase1Mode = "cannon"
						getgenv()._phase1Timer = now
					end

					local function getCannonModel()
						local walls = workspace:FindFirstChild("Climbable") and workspace.Climbable:FindFirstChild("Walls")
						if not walls then return nil, nil, nil end
						for _, wall in ipairs(walls:GetChildren()) do
							local cannons = wall:FindFirstChild("Cannons")
							if cannons then
								local c = cannons:FindFirstChild("1")
								if c then
									local barrel = c:FindFirstChild("Barrel", true)
									local base = c:FindFirstChild("Base", true)
									return c, barrel, base
								end
							end
						end
						return nil, nil, nil
					...
					if getgenv()._phase1Mode == "cannon" then
						UpdateStatus("Phase 1: CANNON BARRAGE on Colossal!")
						root.AssemblyLinearVelocity = V3_ZERO

						task.spawn(function()
							if getgenv()._cannonFiring then return end
							getgenv()._cannonFiring = true
							local cannon, barrel, base = getCannonModel()
							if cannon and barrel and base then
								pcall(function()
								...
								while getgenv()._phase1Mode == "cannon" and self._running do
									pcall(function()
										getRemote:InvokeServer("Cannon", "Shoot", {BarrelWood = barrel, Base = base})
									end)
									task.wait(0.02)
								end
							end
						...
						UpdateStatus("Phase 1: Defending Eren + Killing Titans!")
						...
						-- Multi-hit all titans near Eren
