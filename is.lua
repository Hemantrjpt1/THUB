		...
		while self._running do
			...
			if isColossalRaid then
				...
				if isColossalPhase1 then
					...
					-- 1. CANNON FIRE on Colossal (parallel task)
					if cannonHitbox and colossalNape and not colossalDead then
						task.spawn(function()
							if getgenv()._cannonFiring then return end
							getgenv()._cannonFiring = true

							-- Mount cannon if not mounted
							if not cannonMounted and cannonInteract then
								local icon = cannonInteract:FindFirstChild("Icon", true)
								if icon then
									root.AssemblyLinearVelocity = V3_ZERO
									root.CFrame = CFrame.new(cannonHitbox.Position + Vector3.new(0, 3, 0))
									task.wait(0.3)
									local gs = game:GetService("GuiService")
									local v = game:GetService("VirtualInputManager")
									gs.SelectedObject = icon
									task.wait(0.05)
							...
							-- Fire cannon multi-hit (30 rapid shots)
							local v = game:GetService("VirtualInputManager")
							for _ = 1, 30 do
					...
					if erenPos and #validNapes > 0 then
						-- Go to Eren area and attack titans
						local targetHeightPos = erenPos + Vector3.new(0, getgenv().AutoFarmConfig.HeightOffset, 0)
					...
					-- 3. ALSO multi-hit Colossal Nape directly
					if colossalNape and not colossalDead and (now - lastAttack) >= getgenv().AutoFarmConfig.AttackCooldown then
						lastAttack = now
						if slotData.Weapon == "Blades" then
							for _ = 1, 10 do
								postRemote:FireServer("Attacks", "Slash", true)
