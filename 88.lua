...
function AutoFarm:Start()
	...
	task.spawn(function()
		...
		while self._running do
			...
			-- ==========================================
			-- COLOSSAL RAID: Phase 1 Cannon (Parallel)
			-- ==========================================
			if isColossalRaid then
				...
				if not stallDone then
					UpdateStatus("Colossal Raid - Phase 1: Stalling...")

					-- Start cannon system only ONCE (fully parallel, never touches root.CFrame)
					if not getgenv()._colossalCannonRunning then
						getgenv()._colossalCannonRunning = true
						task.spawn(function()

							-- Helper: find any available cannon on the wall
							...
							-- Helper: always get LIVE nape position (Colossal moves!)
							...
							local cannonModel = nil
							local impactConn = nil
							local cannonRef = workspace:FindFirstChild("Cannon") -- permanent workspace ref

							-- Hook: server fires "Skills","Impact" when cannon hits → we spam S_Skills Impact back
							-- This is the CORRECT trigger (server-validated) with live nape position
							impactConn = postRemote.OnClientEvent:Connect(function(...)
								local args = {...}
								if not getgenv()._colossalCannonRunning then return end
								if args[1] ~= "Skills" or args[2] ~= "Impact" then return end
								-- Refresh cannonRef each time
								local ref = workspace:FindFirstChild("Cannon")
								if not ref then return end
								-- Spam 30 hits with LIVE nape position (Colossal may have moved)
								for i = 1, 30 do
							...
							-- Main cannon shoot loop
							while self._running and not isStallDone() do
								if not cannonModel or not cannonModel.Parent then
									cannonModel = findCannon()
								end
								if not cannonModel then task.wait(1) continue end

								-- Mount cannon (no player TP — server just needs to know we're using it)
								local mountResult = getRemote:InvokeServer("Cannon", "State", cannonModel, true)
								if not mountResult then task.wait(1) continue end

								-- Fire — BarrelWood value from State result, +40 for aim offset
								local barrelWood = (type(mountResult) == "table" and mountResult.BarrelWood) or 0
								local baseVal    = (type(mountResult) == "table" and mountResult.Base) or 0
								getRemote:InvokeServer("Cannon", "Shoot", {BarrelWood = barrelWood + 40, Base = baseVal})

								-- Fallback: also spam Impact directly in case OnClientEvent misses
								-- (e.g. Colossal too close and Shoot misses physically)
								cannonRef = workspace:FindFirstChild("Cannon")
								if cannonRef then
									for i = 1, 15 do
										local napePos = getLiveNapePos()
										if napePos then
											postRemote:FireServer("S_Skills", "Impact", cannonRef, napePos)
										end
									end
								end

								-- Unmount and wait for cannon cooldown
								pcall(function() getRemote:InvokeServer("Cannon", "State", cannonModel, false) end)
								task.wait(2) -- ~1.9s cooldown from cannon attribute
							end

							-- Cleanup
							if impactConn then impactConn:Disconnect() end
							getgenv()._colossalCannonRunning = false
						end)
					end

					-- *** Main loop falls through here ***
					-- → kills regular titans for Eren defense (no conflict, cannon loop never moves player)

				else
					-- Phase 2: Stall done → kill Colossal nape with blades/spears normally
					getgenv()._colossalCannonRunning = false
