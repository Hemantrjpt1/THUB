		...
		while self._running do
			...
			if isColossalRaid then
				...
				if not stallDone then
					...
					-- Start cannon loop only once (parallel, doesn't block titan farming)
					if not getgenv()._colossalCannonRunning then
						...
						task.spawn(function()
							...
							local function getColossalNapePos()
								...
								if hit then
									...
									if nape then return nape.Position end
								end
							...
							while self._running do
								-- Re-check stall objective each cycle
								...
								local done = so and so.Value >= (so:GetAttribute("Requirement") or 1)
								if done then break end

								local cannonModel = findCannon()
								if not cannonModel then task.wait(1) continue end

								-- Step 1: Mount cannon & TP player onto it
								local mountResult = getRemote:InvokeServer("Cannon", "State", cannonModel, true)
								if not mountResult then task.wait(1) continue end

								local base = cannonModel:FindFirstChild("Base") or cannonModel:FindFirstChildWhichIsA("BasePart")
								if base and root then
									root.CFrame = base.CFrame * CFrame.new(0, 4, 0)
								end
								task.wait(0.2)
								...
								local shootOk = getRemote:InvokeServer("Cannon", "Shoot", {BarrelWood = barrelWood + 40, Base = baseVal})

								if shootOk then
									-- Step 3: Wait for cannonball to appear in workspace
									local cannonBall = nil
									local t0 = os.clock()
									repeat
										cannonBall = workspace:FindFirstChild("Cannon")
										task.wait()
									until cannonBall or (os.clock() - t0 > 2)

									-- Step 4: Multi-hit spam on Colossal nape (same as thunder spears)
									if cannonBall then
										local napePos = getColossalNapePos()
										if napePos then
											for i = 1, 30 do
												postRemote:FireServer("S_Skills", "Impact", cannonBall, napePos)
											end
										end
									end
								end

								-- Step 5: Unmount & wait for cannon cooldown
								pcall(function() getRemote:InvokeServer("Cannon", "State", cannonModel, false) end)
								task.wait(2) -- cannon cooldown ~1.9s
							end
					...
					-- Main loop continues → kills titans for Eren defense (fall through)

				else
					-- Phase 2: Stall done → kill Colossal nape normally
					getgenv()._colossalCannonRunning = false -- make sure cannon loop stops
					UpdateStatus("Colossal Raid - Phase 2: Kill Colossal!")
					-- Fall through to normal attack loop (Colossal_Titan is in bossNames)
				end
			end
