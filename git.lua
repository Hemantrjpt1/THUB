...
function AutoFarm:Start()
	...
	task.spawn(function()
		...
		while self._running do
			...
			-- Colossal_Titan is directly inside Titans folder (single nested)
			...
			if isColossalRaid then
				...
				local isPhase1 = defendErenObj ~= nil
				local isPhase2 = colossalBossObj ~= nil and not defendErenObj
					...
					for _, wall in ipairs(walls:GetChildren()) do
						...
						if cannons then
							for _, c in ipairs(cannons:GetChildren()) do
								if c:IsA("Model") then
									local bw = c:FindFirstChild("Barrel") and c.Barrel:FindFirstChild("BarrelWood")
									local ba = c:FindFirstChild("Base")
									if bw and ba then return c, bw, ba end
								end
							end
						end
					end
				...
				if isPhase1 then
					-- Cycle: 2s cannon → 5s defend Eren
					if not getgenv()._p1Timer then
						getgenv()._p1Timer = now
						getgenv()._p1Mode = "cannon"
					end

					local elapsed = now - getgenv()._p1Timer
					...
					if getgenv()._p1Mode == "cannon" then
						UpdateStatus("Phase 1: CANNON BARRAGE!")
							...
							if titansFolder then
								for _, v in ipairs(titansFolder:GetChildren()) do
									...
									if hit then
										...
										if nape then table.insert(napes, nape) end
									end
								end
							end
							for i = 1, math.min(#napes, 8) do
								local nape = napes[i]
								...
								if slotData.Weapon == "Blades" then
									...
									local sl = frame7 and frame7:FindFirstChild("Spears") and frame7.Spears:FindFirstChild("Spears")
									local ammo = sl and tonumber(sl.Text:match("(%d+)")) or 0
									if ammo > 0 then
										getRemote:InvokeServer("Spears", "S_Fire", tostring(ammo))
										for _ = 1, 20 do postRemote:FireServer("Spears", "S_Explode", nape.Position) end
									end
								end
							end
						end
				...
				elseif isPhase2 then
					UpdateStatus("Phase 2: Killing Colossal Titan!")
					...
					if colossalNape and not colossalDead then
						...
						if (now - lastAttack) >= 0.1 then
							...
							if slotData.Weapon == "Blades" then
								...
								if ammo > 0 then
									getRemote:InvokeServer("Spears", "S_Fire", tostring(ammo))
									for _ = 1, 30 do postRemote:FireServer("Spears", "S_Explode", colossalNape.Position) end
								end
							end
						end
					else
