...
function AutoFarm:Start()
	...
	task.spawn(function()
		...
		while self._running do
			...
			-- ================================================================
			-- COLOSSAL RAID LOGIC (from RemoteSpy log)
			-- workspace.Titans.Colossal_Titan
			-- Phase 1: Defend_Eren_2 exists -> Cannon fire + Defend Eren
			-- Phase 2: Colossal_Boss exists -> Attack Colossal nape
			-- ================================================================
			local colossalTitan = titansFolder and titansFolder:FindFirstChild("Colossal_Titan")
			if colossalTitan then
				local colossalNape = colossalTitan:FindFirstChild("Hitboxes")
					and colossalTitan.Hitboxes:FindFirstChild("Hit")
					and colossalTitan.Hitboxes.Hit:FindFirstChild("Nape")
				local colossalDead = colossalTitan:GetAttribute("Dead") or colossalTitan:GetAttribute("Killed")

				local defendErenObj = ws_ObjectiveFolder and ws_ObjectiveFolder:FindFirstChild("Defend_Eren_2")
				local colossalBossObj = ws_ObjectiveFolder and ws_ObjectiveFolder:FindFirstChild("Colossal_Boss")

				-- Cannon path confirmed from log
				local cannon = workspace:FindFirstChild("Climbable")
				...
				if defendErenObj then
					-- PHASE 1: 2s cannon, 5s defend Eren cycle
					if not getgenv()._p1Timer then
						getgenv()._p1Timer = os.clock()
						getgenv()._p1Mode = "cannon"
						getgenv()._cannonFiring = false
					end

					local elapsed = os.clock() - getgenv()._p1Timer
					if getgenv()._p1Mode == "cannon" and elapsed >= 2 then
						getgenv()._p1Mode = "defend"
						getgenv()._p1Timer = os.clock()
						getgenv()._cannonFiring = false
					elseif getgenv()._p1Mode == "defend" and elapsed >= 5 then
						getgenv()._p1Mode = "cannon"
						getgenv()._p1Timer = os.clock()
					end

					if getgenv()._p1Mode == "cannon" then
						UpdateStatus("Phase 1: Cannon firing!")
						...
						task.spawn(function()
							...
							if cannon then
								...
								task.wait(0.1)
								while getgenv()._p1Mode == "cannon" and AutoFarm._running do
									-- Fire cannon
									pcall(function()
										getRemote:InvokeServer("Cannon", "Shoot", {BarrelWood = 40, Base = 0})
									end)
									-- Actual damage via S_Skills Impact (from log)
									local ct = workspace:FindFirstChild("Titans") and workspace.Titans:FindFirstChild("Colossal_Titan")
									local nape = ct and ct:FindFirstChild("Hitboxes")
										and ct.Hitboxes:FindFirstChild("Hit")
										and ct.Hitboxes.Hit:FindFirstChild("Nape")
									local cannonBall = workspace:FindFirstChild("Cannon")
									if cannonBall and nape then
										for _ = 1, 10 do
											pcall(function()
												postRemote:FireServer("S_Skills", "Impact", cannonBall, nape.Position)
											end)
										end
									end
									task.wait(2)
						...
						-- DEFEND EREN: hover near Eren, kill titans
						UpdateStatus("Phase 1: Defending Eren!")
						local erenCollider = defendErenObj:FindFirstChild("Collider")
						if erenCollider then
							local targetPos = erenCollider.Position + Vector3.new(0, 80, 30)
						...
						if (now - lastAttack) >= 0.1 then
							...
							for i = 1, math.min(#validNapes, 8) do
								...
								if slotData.Weapon == "Blades" then
									...
									local f7 = PlayerGui.Interface.HUD.Main.Top["7"]
									local sl = f7 and f7:FindFirstChild("Spears") and f7.Spears:FindFirstChild("Spears")
									local ammo = sl and tonumber(sl.Text:match("(%d+)")) or 0
									if ammo > 0 then
				...
				elseif colossalBossObj then
					-- PHASE 2: Attack Colossal nape directly
					UpdateStatus("Phase 2: Kill Colossal!")
					getgenv()._p1Timer = nil
					getgenv()._p1Mode = nil
					getgenv()._cannonFiring = false
