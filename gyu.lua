...
function AutoFarm:Start()
	...
	task.spawn(function()
		...
		while self._running do
			...
			-- ==========================================
			-- COLOSSAL RAID: Phase 1 — One-shot Stall Burst
			-- ==========================================
			if isColossalRaid then
				...
				if not stallDone then
					UpdateStatus("Colossal Raid - Phase 1: Stalling Colossal...")
					...
					if not getgenv()._colossalCannonRunning then
						...
						task.spawn(function()
							...
							-- Find and mount cannon
							local cannonModel = findCannon()
							if not cannonModel then
							...
							-- Grab workspace.Cannon reference (permanent object)
							local cannonRef = workspace:FindFirstChild("Cannon")

							-- BURST: spam S_Skills Impact until stall objective completes
							-- Each iteration fires with LIVE nape position (Colossal moves!)
							local shotCount = 0
							while self._running and not isStallDone() do
								local napePos = getLiveNapePos()
								if napePos and cannonRef then
									-- Fire 50 impacts per frame, then yield so server processes
									for i = 1, 50 do
										postRemote:FireServer("S_Skills", "Impact", cannonRef, napePos)
									end
									shotCount = shotCount + 50
									UpdateStatus("Colossal Raid - Stalling... (" .. shotCount .. " hits)")
								end
								-- Refresh cannonRef each cycle in case it changed
								cannonRef = workspace:FindFirstChild("Cannon")
								-- Re-shoot every ~2s to keep cannon "active" server-side
								if shotCount % 100 == 0 then
									local newMount = getRemote:InvokeServer("Cannon", "State", cannonModel, true)
									if newMount then
										local bw = (type(newMount) == "table" and newMount.BarrelWood) or 0
										local bv = (type(newMount) == "table" and newMount.Base) or 0
										getRemote:InvokeServer("Cannon", "Shoot", {BarrelWood = bw + 40, Base = bv})
									end
								end
					...
					-- Phase 1: skip regular titan loop (focus = stalling Colossal)
					task.wait()
					continue

				else
					-- Phase 2: Stall done → normal nape attack
					getgenv()._colossalCannonRunning = false
