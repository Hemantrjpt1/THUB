...
function AutoFarm:Start()
	...
	task.spawn(function()
		...
		while self._running do
			...
			-- ==========================================
			-- COLOSSAL RAID: Phase 1 Cannon Stall Logic
			-- ==========================================
			if isColossalRaid then
				...
				if not stallDone then
					-- Phase 1: Use cannon to stall Colossal + kill titans for Eren defense
					...
					local function getColossalNapePos()
						...
						if ct then
							...
							local head = ct:FindFirstChild("Head") or (ct:FindFirstChild("Fake") and ct.Fake:FindFirstChild("Head"))
					...
					if cannonModel then
						...
						if mountResult then
							...
							-- Fire cannon
							local barrelWood = (mountResult and mountResult.BarrelWood) or 0
							local baseVal = (mountResult and mountResult.Base) or 0
							local shootResult = getRemote:InvokeServer("Cannon", "Shoot", {BarrelWood = barrelWood + 40, Base = baseVal})
							
							if shootResult then
								-- Wait for cannon projectile to spawn
								local cannonBall = nil
								local waitStart = os.clock()
								repeat
									cannonBall = workspace:FindFirstChild("Cannon")
									task.wait()
								until cannonBall or (os.clock() - waitStart > 2)

								if cannonBall then
									local napePos = getColossalNapePos()
									if napePos then
										-- Multi-hit Impact spam (like Thunder Spears S_Explode)
										for i = 1, 30 do
							...
							-- Unmount / wait for cooldown
							pcall(function() getRemote:InvokeServer("Cannon", "State", cannonModel, false) end)
							task.wait(1.9) -- cannon cooldown
						end
					end

					-- Also kill regular titans nearby (Eren defense)
					-- Fall through to normal titan loop below
				else
					-- Phase 2: Stall done, attack Colossal nape normally
