...
function AutoFarm:Start()
	...
	task.spawn(function()
		...
		while self._running do
			...
			-- Colossal Raid detection
			local colossalTitan = (workspace:FindFirstChild("Titans") and workspace.Titans:FindFirstChild("Colossal_Titan")) or workspace:FindFirstChild("Colossal_Titan")
			local isColossalRaid = colossalTitan ~= nil
			if isColossalRaid then
				getgenv().AutoFarmConfig.HeightOffset = math.max(getgenv().AutoFarmConfig.HeightOffset, 350)
			end
			...
			-- ==========================================
			-- COLOSSAL RAID PHASE DETECTION
			-- ==========================================
			local colossalObjective = ws_ObjectiveFolder and ws_ObjectiveFolder:FindFirstChild("Colossal_Boss")
			local defendErenObjective = ws_ObjectiveFolder and ws_ObjectiveFolder:FindFirstChild("Defend_Eren_2")
			local isColossalPhase1 = isColossalRaid and defendErenObjective ~= nil
			local isColossalPhase2 = isColossalRaid and colossalObjective ~= nil and not defendErenObjective
			...
			-- ==========================================
			-- COLOSSAL RAID PHASE 1 + PHASE 2 LOGIC
			-- ==========================================
			if isColossalRaid then
				colossalTitan = (workspace:FindFirstChild("Titans") and workspace.Titans:FindFirstChild("Colossal_Titan")) or workspace:FindFirstChild("Colossal_Titan")
				local colossalNape = colossalTitan and colossalTitan:FindFirstChild("Hitboxes")
					and colossalTitan.Hitboxes:FindFirstChild("Hit")
					and colossalTitan.Hitboxes.Hit:FindFirstChild("Nape")
				local colossalHRP = colossalTitan and colossalTitan:FindFirstChild("HumanoidRootPart")
				local colossalDead = colossalTitan and (colossalTitan:GetAttribute("Dead") or colossalTitan:GetAttribute("Killed"))

				-- Get cannon
				local cannonModel = workspace:FindFirstChild("Climbable")
					and workspace.Climbable:FindFirstChild("Walls")
					and workspace.Climbable.Walls:FindFirstChild("Wall")
					and workspace.Climbable.Walls.Wall:FindFirstChild("Cannons")
					and workspace.Climbable.Walls.Wall.Cannons:FindFirstChild("1")
				local cannonHitbox = cannonModel and cannonModel:FindFirstChild("Hitbox")
				local cannonInteract = PlayerGui:FindFirstChild("Interact_1")
				local cannonMounted = PlayerGui.Interface:FindFirstChild("Cannon") and PlayerGui.Interface.Cannon.Visible

				if isColossalPhase1 then
					UpdateStatus("Phase 1: Cannon + Multi-Hit Colossal + Defend Eren")
					...
					local function getCannonModel()
						...
						for _, wall in ipairs(walls:GetChildren()) do
							...
							if cannons then
								local c = cannons:FindFirstChild("1")
					...
					-- Track burst count
					getgenv()._colossalBurstCount = getgenv()._colossalBurstCount or 0
					getgenv()._erenDefendMode = getgenv()._erenDefendMode or false
					...
					-- Switch modes: after 3 bursts defend Eren, after defending go back to attack
					if getgenv()._erenDefendMode then
						-- DEFEND EREN MODE
						UpdateStatus("Phase 1: Defending Eren!")
						if erenPos then
							local targetPos = erenPos + Vector3.new(0, 80, 30)
						...
						if (now - lastAttack) >= 0.1 then
							lastAttack = now
							for i = 1, math.min(#validNapes, 8) do
								local nape = validNapes[i]
								if not nape or not nape.Parent then continue end
								if slotData.Weapon == "Blades" then
									for _ = 1, 5 do
									...
									local spearsLabel = frame7 and frame7:FindFirstChild("Spears") and frame7.Spears:FindFirstChild("Spears")
									local ammo = spearsLabel and tonumber(spearsLabel.Text:match("(%d+)")) or 0
									if ammo > 0 then
						...
						-- After 5s of defending, go back to attack Colossal
						getgenv()._erenDefendTimer = (getgenv()._erenDefendTimer or 0) + 1
						if getgenv()._erenDefendTimer >= 50 then -- ~5s at 0.1s loop
							getgenv()._erenDefendMode = false
							getgenv()._erenDefendTimer = 0
							getgenv()._colossalBurstCount = 0
						end

					else
						-- ATTACK COLOSSAL MODE
						UpdateStatus("Phase 1: Multi-Hit Colossal!")

						-- Move to Colossal nape
						if colossalNape and not colossalDead then
							local targetPos = colossalNape.Position + Vector3.new(0, 50, 0)
							local dir = targetPos - root.Position
							root.AssemblyLinearVelocity = dir.Magnitude > 1 and dir.Unit * getgenv().AutoFarmConfig.MoveSpeed or V3_ZERO
						end

						if (now - lastAttack) >= 0.05 then
							lastAttack = now
							getgenv()._colossalBurstCount = getgenv()._colossalBurstCount + 1
							...
							-- DIRECT multi-hit Colossal nape (50 hits)
							if colossalNape and not colossalDead then
								if slotData.Weapon == "Blades" then
									for _ = 1, 50 do
										postRemote:FireServer("Attacks", "Slash", true)
										postRemote:FireServer("Hitboxes", "Register", colossalNape, math.random(625, 850))
									end
								else
									local frame7 = PlayerGui.Interface.HUD.Main.Top["7"]
									local spearsLabel = frame7 and frame7:FindFirstChild("Spears") and frame7.Spears:FindFirstChild("Spears")
									...
									if ammo > 0 then
										getRemote:InvokeServer("Spears", "S_Fire", tostring(ammo))
										for _ = 1, 50 do
											postRemote:FireServer("Spears", "S_Explode", colossalNape.Position)
										end
									end
								end
							end

							-- After 3 bursts, switch to Eren defend mode
							if getgenv()._colossalBurstCount >= 3 then
								getgenv()._erenDefendMode = true
								getgenv()._erenDefendTimer = 0
							end
						end
					end
				...
				elseif isColossalPhase2 then
					UpdateStatus("Phase 2: Attacking Colossal Titan Nape!")

					if colossalNape and not colossalDead then
						-- Hover above Colossal nape
						local targetPos = colossalNape.Position + Vector3.new(0, 50, 0)
						local dir = targetPos - root.Position
						root.AssemblyLinearVelocity = dir.Magnitude > 1 and dir.Unit * getgenv().AutoFarmConfig.MoveSpeed or V3_ZERO

						-- Multi-hit Colossal Nape
						if (now - lastAttack) >= getgenv().AutoFarmConfig.AttackCooldown then
							lastAttack = now
							if slotData.Weapon == "Blades" then
								for _ = 1, 15 do
									postRemote:FireServer("Attacks", "Slash", true)
								...
								local ammo = tonumber(spearsLabel.Text:match("(%d+)")) or 0
								if ammo > 0 then
									getRemote:InvokeServer("Spears", "S_Fire", tostring(ammo))
									for _ = 1, 30 do
										postRemote:FireServer("Spears", "S_Explode", colossalNape.Position)
			...
			-- ==========================================
			-- END COLOSSAL RAID LOGIC
			-- ==========================================

			if targetPart then
				UpdateStatus(closestIsBoss and "Attacking Boss..." or "Farming Titans...")
				-- Traverse up to find the root Titan Model
				local currentTitanModel = targetPart
				while currentTitanModel and currentTitanModel.Parent ~= titansFolder and currentTitanModel.Parent ~= workspace do
