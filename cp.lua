...
function AutoFarm:Start()
	...
	task.spawn(function()
		...
		local function checkReady()
			...
			-- Colossal Titan raid: check workspace directly
			if not titansReady then
				local colossal = workspace:FindFirstChild("Colossal_Titan")
				if colossal and colossal:FindFirstChild("Fake") and colossal.Fake:FindFirstChild("Head") and colossal.Fake.Head:FindFirstChild("Header") then
					titansReady = true
				end
			end
		...
		while self._running do
			...
			-- Colossal Titan raid: nape is at Y~640, use higher offset
			local isColossalRaid = workspace:FindFirstChild("Colossal_Titan") ~= nil
			if isColossalRaid then
				getgenv().AutoFarmConfig.HeightOffset = math.max(getgenv().AutoFarmConfig.HeightOffset, 300)
			end
			...
			if now >= nextTitanCacheUpdate then
				...
				-- Normal titans (in Titans folder)
				...
				-- Colossal Titan (exists directly in workspace, not Titans folder)
				local colossalTitan = workspace:FindFirstChild("Colossal_Titan")
				if colossalTitan and not colossalTitan:GetAttribute("Killed") and not colossalTitan:GetAttribute("Dead") then
					local hit = colossalTitan:FindFirstChild("Hitboxes") and colossalTitan.Hitboxes:FindFirstChild("Hit")
					if hit then
						local nape = hit:FindFirstChild("Nape")
						if nape then table.insert(validNapes, nape) end
					end
			...
			for i = 1, #validNapes do
				...
				-- Traverse up to find titan model (works for both Titans folder and workspace-level)
				local titanModel = nape.Parent.Parent.Parent
				-- Colossal_Titan: nape.Parent = Hit, nape.Parent.Parent = Hitboxes, nape.Parent.Parent.Parent = Colossal_Titan
				-- Regular titan: nape.Parent = Hit, nape.Parent.Parent = Hitboxes, nape.Parent.Parent.Parent = TitanModel
				...
				local isColossal = tName == "Colossal_Titan"
				...
				-- Colossal Titan: only attack when State attribute exists (active/spawned)
				if isColossal then
					if not titanModel:GetAttribute("State") then continue end
				elseif isBoss then
					if not titanModel:GetAttribute("State") then continue end
				end
	...
	if getgenv().AutoStart and game.PlaceId == 14916516914 then
		task.spawn(function()
			...
			while getgenv().AutoStart do
				...
				local isMission = (missionType == "Missions" or missionType == 1)
...
Options.RaidMapDropdown:OnChanged(function()
	...
	if not Value then return end
	Options.RaidObjectiveDropdown:SetValues(Raids[Value] or {})
...
local initRaidObjVals = Raids[initRaidMap] or {}
